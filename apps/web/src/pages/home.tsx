...
export default function Home() {
  ...
  const { data: albumsData } = useListAlbums({ featured: true, limit: 4 } as { limit?: number });
  const { data: testimonialsData } = useListTestimonials();
  const VISIBLE = 3;
  const { visible, total, index: tIndex, direction: tDir, prev, next, goTo } =
    useTestimonialsCarousel(testimonialsData?.testimonials ?? []);

  // Avoid flash of DEFAULT_HERO_VIDEO (Mixkit) on load: only use video when we have artistInfo
  const heroVideoUrl =
    artistInfo === undefined
      ? null
      : (artistInfo?.heroVideoUrl?.trim() || DEFAULT_HERO_VIDEO);
  const fallbackPoster = artistInfo?.heroPosterUrl?.trim() || `${import.meta.env.BASE_URL}images/hero-bg.png`;
        ...
        <div className="absolute inset-0 z-0">
          {youtubeId ? (
            ...
            <img
              src={fallbackPoster}
              alt=""
              className={`w-full h-full object-cover transition-all duration-700 ease-out ${artistInfo === undefined ? "blur-2xl scale-105" : "blur-0 scale-100"}`}
              loading="eager"
            />
          )}
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto mt-20">
          ...
          >
            <h1 className="font-display text-5xl md:text-7xl lg:text-8xl font-bold text-foreground mb-6 uppercase tracking-[0.1em] leading-tight">
              {(() => {
                const prefix = artistInfo === undefined ? DEFAULT_HERO.prefix : (artistInfo?.heroTitlePrefix?.trim() ?? "");
                const highlight = artistInfo === undefined ? DEFAULT_HERO.highlight : (artistInfo?.heroTitleHighlight?.trim() ?? "");
                const suffix = artistInfo === undefined ? DEFAULT_HERO.suffix : (artistInfo?.heroTitleSuffix?.trim() ?? "");
                return (
                  <>
                    {prefix}
            ...
            <p className="text-xl md:text-2xl text-muted-foreground mb-10 font-light tracking-wide max-w-2xl mx-auto">
              {artistInfo === undefined ? DEFAULT_HERO_TAGLINE : (artistInfo?.heroTagline?.trim() ?? "")}
            </p>
