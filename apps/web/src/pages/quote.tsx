...
const SERVICE_TYPES = ["mixing", "mastering", "stem-mastering", "production", "mixing-mastering", "string-arrangement", "cowriting", "guitar-solo"] as const;
...
function getServiceFromUrl(): string | null {
  if (typeof window === "undefined") return null;
  const p = new URLSearchParams(window.location.search).get("service");
  return p && SERVICE_TYPES.includes(p as (typeof SERVICE_TYPES)[number]) ? p : null;
}

export default function QuoteBuilder() {
  const [step, setStep] = useState(1);
  const addItem = useCartStore(state => state.addItem);
  const preselectedService = useMemo(() => getServiceFromUrl(), []);
