...
// §7: zero server data + client-only (cart, form, useCreateOrder mutation) → this
// page.tsx is legitimately a Client Component. Mock checkout preserved (§4: Stripe
// deferred — same client-supplied prices, order.status stays "pending").
...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = sessionStorage.getItem(BOOKING_CONTACT_KEY);
      if (stored) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only run when hasBooking changes
  }, [hasBooking]);
  ...
  const onSubmit = (data: CheckoutForm) => {
    const orderItems = items.map((item) => ({
      ...
      bookingId: item.type === "booking" ? item.bookingId ?? null : null,
    }));
    ...
    createOrder({
      ...
      onSuccess: () => {
        clearCart();
        try {
          sessionStorage.removeItem(BOOKING_CONTACT_KEY);
        } catch {
          // ignore
        }
      }
        ...
        <div className="lg:col-span-7">
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <div className="glass-panel p-8 space-y-6">
              ...
              {hasBooking && (
                <div>
                  <label className="text-sm font-bold text-muted-foreground uppercase tracking-wider block mb-2">WhatsApp (optional)</label>
                  <input
                    type="tel"
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-sm p-4 text-white focus:border-primary outline-none transition-colors"
                  />
                  <p className="text-muted-foreground text-xs mt-1">Include country code + number. Tom will contact you via WhatsApp or Email to coordinate your session.</p>
                </div>
              )}
            ...
            <div className="glass-panel p-8">
              <h2 className="text-xl font-bold text-muted-foreground uppercase tracking-wider mb-6">Payment Method</h2>
