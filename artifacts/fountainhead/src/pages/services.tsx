...
const CATEGORY_META: Record<string, { icon: React.ElementType; color: string; label: string }> = {
  ...
  lesson:       { icon: BookOpen,  color: "text-secondary", label: "Lesson" },
  coaching:     { icon: Lightbulb, color: "text-secondary", label: "Coaching" },
  studio:       { icon: Mic2,      color: "text-secondary", label: "Studio" },
};

const STUDIO_CATEGORIES = new Set(["mixing", "mastering", "production", "guitar-solo"]);

export default function Services() {
  const { data, isLoading } = useListServices();

  const studioServices = data?.services?.filter(s => STUDIO_CATEGORIES.has(s.category)) ?? [];
  const sessionServices = data?.services?.filter(s => !STUDIO_CATEGORIES.has(s.category)) ?? [];
...
function ServiceCard({ service }: { service: { id: number; name: string; description: string; category: string; priceEur: string; unit: string } }) {
  const meta = CATEGORY_META[service.category] ?? { icon: Music4, color: "text-primary", label: service.category };
  const Icon = meta.icon;
  const isStudio = STUDIO_CATEGORIES.has(service.category);
    ...
    <div className="group relative flex flex-col bg-card border border-white/6 rounded-sm overflow-hidden hover:border-primary/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
      ...
      <div className="p-7 flex flex-col flex-1">
        ...
        {isStudio ? (
          ...
          <Link href="/book">
            <Button variant="outline" className="w-full gap-2 group-hover:border-primary group-hover:text-primary transition-all">
